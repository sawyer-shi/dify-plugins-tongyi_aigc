...
基于参考视频风格生成视频。
- **支持模型**: wan2.6-r2v, wan2.6-r2v-flash
- **功能特性**:
  - 参考视频或图像输入（最多5个）
  - 时长：2-10 秒
...
根据文本和参考图像生成图像。
- **参数**:
  - `model`: 模型版本（默认：wan2.6-image）
  - `prompt`: 文本描述（必需，<=2000字符）
  - `images`: 参考图片文件（1-4张，必需）
  - `size`: 图像尺寸（默认：1280*1280）
  - `n`: 生成图像数量（1-4，默认：3）
...
基于参考视频风格生成视频。
- **参数**:
  - `model`: 模型版本（默认：wan2.6-r2v）
  - `prompt`: 文本描述（必需，最多1500字符）
  - `reference_urls`: 参考 URL（视频或图片，最多5个）
  - `size`: 视频分辨率（默认：1920*1080）
  - `duration`: 时长（秒）（2-10，默认：5）
  - `shot_type`: 单镜头或多镜头（默认：单镜头）
